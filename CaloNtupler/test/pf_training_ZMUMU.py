...
class TMVA_Analysis():
    def __init__(self,options):
        ...
        if options.new:
           ...
           self._lSigFile.cd()
           self._lSigTree       = self._lInputTree.CopyTree("match==1 && std::abs(eta) >= 1.7 && std::abs(eta) <= 3.0 && pt > 5")
           self._lSigTree.Write()
           self._lSigFile.Close() 
           self._lBkgFile.cd()
           self._lBkgTree       = self._lInputTree.CopyTree("match==0 && std::abs(eta) >= 1.7 && std::abs(eta) <= 3.0 && pt > 5")
        ...
        self._lFactory.BookMethod(self._lDataLoader, TMVA.Types.kPyKeras, 'PyKeras', 'H:!V:FilenameModel=model.h5:NumEpochs=5:BatchSize=100')
