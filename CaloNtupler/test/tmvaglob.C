...
namespace TMVAGlob {
   ...
   void imgconv( TCanvas* c, const TString & fname )
   {
      ...
      if (NULL == c) {
         ...
         if (UsePaperStyle) {
            ...
            cout << "--- --------------------------------------------------------------------" << endl;
            cout << "--- If you want to save the image as png, pdf, eps or gif, please" << endl;
            cout << "--- comment out the corresponding lines (line no. 247-250) in tmvaglob.C" << endl;
            cout << "--- --------------------------------------------------------------------" << endl;
            c->Print(pngName);
            c->Print(pdfName);
            // c->Print(gifName);
   ...
   Int_t GetNumberOfInputVariables( TDirectory *dir )
   {
      ...
      while ((key = (TKey*)next())) {
         ...
         if (keyName.Contains("__Signal") || (keyName.Contains("__Regression") && !(keyName.Contains("__Regression_target")))) noVars++;
      }
